...
class CodeAgent:
    ...
    def step(self, game_state):
        ...
        try:
            ...
            if self.last_execution_error or is_stuck or self.last_generated_code is None or steps_since_generation >= 30:
                # 에러 있거나, Stuck이거나, 첫 실행, 또는 60스텝 경과 -> 새 코드 생성
                ...
                # Not stuck and no error -> 이전 코드 재사용
    ...
    def _generate_new_code(self, game_state, is_stuck: bool) -> str:
        ...
        # 3. State와 milestone 정보 (JSON 형태로 변환)
        ...
        # 4. Knowledge base 가져오기
        knowledge_text = self.knowledge_base.to_prompt_format(limit=20)

        # 5. 프롬프트 생성 (raw code 전달, 포맷팅은 PromptBuilder가 담당)
            ...
            input=[
                {
                    ...
                    "content": (
                        ...
                        if not screenshot_base64
                            ...
                            {"type": "input_text", "text": prompt},
                            {"type": "input_image",
                            "image_url": f"data:image/png;base64,{screenshot_base64}"}  # data URL 허용
                        ]
                    )
                }
            ],
            reasoning={"effort": "low"}  # reasoning 모델에서만 의미 있음
        )
