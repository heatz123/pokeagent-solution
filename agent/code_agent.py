...
class CodeAgent:
        ...
        try:
            ...
            if self.use_subtasks:
                ...
                if steps_since_generation >= 30 and situation == "NORMAL":
                    situation = "STUCK"
                    print(f"⏰ 30 steps passed ({steps_since_generation}) - treating as STUCK")
            ...
            if need_new_code:
                ...
                if self.use_subtasks:
                    ...
                    if situation == "SUCCESS_ACHIEVED" and current_subtask:
                        print(f"✅ Subtask completed: {current_subtask['description']}")
                        self.subtask_manager.clear_current_subtask()
                        current_subtask = None

                    print(f"🤖 Generating new code for {situation} situation...")
                ...
                if self.use_subtasks:
                    ...
                    print("ℹ️ Reusing existing subtask code...")
                else:
    ...
    def _generate_new_code(
        ...
        if analysis_text:
            ...
            print(f"📊 ANALYSIS extracted (count: {len(self.previous_analyses)}/{self.analysis_threshold})")

            # Check if we should summarize analyses
            if self._should_summarize_analyses():
                print(f"🔔 Triggering summarization at {len(self.previous_analyses)} analyses")
                self._summarize_and_reset_analyses()
        else:
            print(f"⚠️ No ANALYSIS section found in LLM response (step {self.step_count})")
    ...
    def _get_recent_screenshots(self, max_count=10):
        ...
        Returns:
            List of (step_count, PIL.Image) tuples (최대 max_count개, 시간순)
        """
    ...
    def _summarize_and_reset_analyses(self):
        ...
        Result: N entries → 1 summary entry
        """
        current_count = len(self.previous_analyses)

        print(f"\n{'='*60}")
        print(f"📝 Analysis count reached {current_count} (threshold: {self.analysis_threshold})")
        ...
        try:
            ...
            print(f"  ✅ Summary created for steps {step_label}")
            print(f"  📉 Context reduced: {current_count} → 1 entry")
            print(f"  💾 Total summaries created: {self.summarizer.total_summaries_created}")
            print(f"{'='*60}\n")

        except Exception as e:
            print(f"  ❌ Summarization failed: {e}")
            print(f"  ⚠️ Keeping original analyses")
            import traceback
            traceback.print_exc()

            # Fallback: truncate to prevent unbounded growth
            if len(self.previous_analyses) > 30:
                print(f"  ⚠️ Truncating to last 30 analyses as fallback")
